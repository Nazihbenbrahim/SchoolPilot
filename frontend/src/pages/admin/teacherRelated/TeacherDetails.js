...
const TeacherDetails = () => {
    ...
    const { subjectsList } = useSelector((state) => state.sclass); // Retrieve subjectsList from Redux
    ...
    const teachClasses = storedMapping ? JSON.parse(storedMapping) : teacherDetails?.teachClasses || [];
                        ...
                        teachClasses.map((cls, index) => (
                            <div key={index} className="mb-4">
                                <p className="text-lg text-gray-700 font-poppins">
                                    Class: {cls.className || teacherDetails?.teachSclasses?.find(c => c._id === cls.classId)?.sclassName || 'N/A'}
                                </p>
                                <ul className="list-disc list-inside text-gray-700 font-poppins">
                                    {cls.subjects?.length > 0 ? (
                                        cls.subjects.map((subjectId, idx) => {
                                            const subject = subjectsList.find(sub => sub._id === subjectId);
                                            return (
                                                <li key={idx}>
                                                    {subject?.subName || subjectId} ({subject?.sessions || 'N/A'} sessions)
                                                </li>
                                            );
                                        })
                                    ) : (
